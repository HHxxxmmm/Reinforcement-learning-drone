"""Batch eval Stage-2 Phase-2 v3 pitch nocap checkpoints (no max_pitch).
...
MODEL_DIR = ROOT / "model/stage2_phase2_v3_pitch_nocap"
ENV_CONFIG = ROOT / "config/envs.stage2.phase2.v3.pitch.nocap.eval.yaml"
OUTPUT_DIR = ROOT / "logs/stage2_phase2_v3_pitch_nocap/eval"
SUMMARY_CSV = OUTPUT_DIR / "phase2_v3_pitch_nocap_summary.csv"
...
def main():
    parser = argparse.ArgumentParser(description="Batch eval Phase-2 v3 pitch nocap checkpoints")
    parser.add_argument("--episodes", type=int, default=3)
    parser.add_argument("--log-interval", type=int, default=0, help="0 = quiet per-step logs")
    parser.add_argument(
        "--checkpoint",
        default="",
        help="Single checkpoint filename or path (default: all in model/stage2_phase2_v3_pitch_nocap/)",
    )
    ...
    print(f"Evaluating {len(checkpoints)} pitch nocap checkpoints × {args.episodes} episodes")
    ...
    print(f"\n=== pitch nocap summary (pitch | yaw | enemy_hp | kills) ===")
    print(f"{'checkpoint':<48} {'pitch_deg':>9} {'yaw_deg':>8} {'enemy_hp':>9} {'kills':>6}")
    by_ckpt = {}
    for row in all_rows:
    ...
    for name in sorted(by_ckpt.keys(), key=_ckpt_sort_key):
        ...
        kills = sum(1 for r in rows if str(r["killed"]).lower() == "true")
        print(f"  {name:<48} {pitch:+9.2f} {yaw:+8.2f} {hp:9.3f} {kills}/{len(rows)}")
