"""Batch eval Stage-2 Phase-2 checkpoints (final yaw + enemy HP).

Example:
    python scripts/eval_stage2_phase2.py --episodes 3
"""
import argparse
import csv
import re
...
MODEL_DIR = ROOT / "model/stage2_phase2"
ENV_CONFIG = ROOT / "config/envs.stage2.phase2.eval.yaml"
OUTPUT_DIR = ROOT / "logs/stage2_phase2/eval"
...
def main():
    parser = argparse.ArgumentParser(description="Batch eval Phase-2 combat checkpoints")
    parser.add_argument("--episodes", type=int, default=3)
    parser.add_argument("--log-interval", type=int, default=0, help="0 = quiet per-step logs")
    parser.add_argument(
        "--checkpoint",
        default="",
        help="Single checkpoint filename or path (default: all in model/stage2_phase2/)",
    )
    ...
    print(f"Evaluating {len(checkpoints)} Phase-2 checkpoints × {args.episodes} episodes")
    ...
    print(f"\n=== Phase-2 summary (final_yaw_deg | enemy_hp | kills) ===")
    print(f"{'checkpoint':<30} {'yaw_deg':>8} {'enemy_hp':>9} {'kills':>6}")
    by_ckpt = {}
    for row in all_rows:
    ...
    for name in sorted(by_ckpt.keys(), key=_ckpt_sort_key):
        ...
        kills = sum(1 for r in rows if str(r["killed"]).lower() == "true")
        print(f"  {name:<30} {yaw:+8.2f} {hp:9.3f} {kills}/{len(rows)}")
