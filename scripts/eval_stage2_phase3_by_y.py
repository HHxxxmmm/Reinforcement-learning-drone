"""Eval Phase-3 checkpoints at fixed enemy y values (-5..5).

Each checkpoint × each y → 1 deterministic episode.
Metrics: final_yaw_deg, enemy_hp_final.
...
MODEL_DIR = ROOT / "model/stage2_phase3"
ENV_CONFIG = ROOT / "config/envs.stage2.phase3.eval.yaml"
OUTPUT_DIR = ROOT / "logs/stage2_phase3/eval"
...
def eval_checkpoint(ckpt: Path, y_values, env_config: str, log_interval: int):
    ...
    for y in y_values:
        ...
        print(
            f"-> yaw={row['final_yaw_deg']:+6.1f}deg hp={row['enemy_hp_final']:.3f} [{kill_tag}]"
        )
    env.adaptor.close()
    return rows
...
def main():
    parser = argparse.ArgumentParser(description="Eval Phase-3 ckpts at fixed enemy y (-5..5)")
    ...
    print(f"{'checkpoint':<35} " + " ".join(f"y{y:+d}" for y in y_values))
    by_ckpt = {}
    for row in all_rows:
        by_ckpt.setdefault(row["checkpoint"], {})[int(row["enemy_y"])] = row
    for name in sorted(by_ckpt.keys(), key=_ckpt_sort_key):
        cells = []
        for y in y_values:
            r = by_ckpt[name].get(y)
            cells.append(f"{r['enemy_hp_final']:.2f}" if r else "  - ")
        kills = sum(1 for r in by_ckpt[name].values() if r["killed"])
        print(f"  {name:<35} " + " ".join(f"{c:>5s}" for c in cells) + f"  K={kills}")
