"""Batch eval Stage-2 Phase-2 v3 checkpoints (P1→P2 + reset yaw).
...
MODEL_DIR = ROOT / "model/stage2_phase2_v3"
ENV_CONFIG = ROOT / "config/envs.stage2.phase2.v3.eval.yaml"
OUTPUT_DIR = ROOT / "logs/stage2_phase2_v3/eval"
SUMMARY_CSV = OUTPUT_DIR / "phase2_v3_summary.csv"
...
def main():
    parser = argparse.ArgumentParser(description="Batch eval Phase-2 v3 combat checkpoints")
    parser.add_argument("--episodes", type=int, default=3)
    parser.add_argument("--log-interval", type=int, default=0, help="0 = quiet per-step logs")
    parser.add_argument(
        "--checkpoint",
        default="",
        help="Single checkpoint filename or path (default: all in model/stage2_phase2_v3/)",
    )
    ...
    print(f"Evaluating {len(checkpoints)} Phase-2 v3 checkpoints × {args.episodes} episodes")
    ...
    print(f"\n=== Phase-2 v3 summary (final_yaw_deg | enemy_hp | kills) ===")
